...
public class PSalesBrandBasedProductAdapter extends RecyclerView.Adapter<PSalesBrandBasedProductAdapter.MyViewHolder> {
    public ArrayList<PSalesBrandBasedProductModel> pSalesBrandBasedProductModel;
    public Context context;
    public static String Target,Sale;

    public PSalesBrandBasedProductAdapter(ArrayList<PSalesBrandBasedProductModel> pSalesBrandBasedProductModel, Context context) {
        this.pSalesBrandBasedProductModel = pSalesBrandBasedProductModel;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder myViewHolder, int i) {
        final PSalesBrandBasedProductModel mPSalesBrandBasedProductModel = pSalesBrandBasedProductModel.get(i);
        myViewHolder.tv_dot.setTextColor(Color.rgb(255, 165, 0));
        myViewHolder.tv_achie.setTextColor(Color.rgb(255, 165, 0));
        myViewHolder.tv_HQName.setText(mPSalesBrandBasedProductModel.getProduct_Name());
...
//        myViewHolder.tv_primary.setText("₹ " + mPSalesHQBasedBrandModel.getPrimary() + "L");
        ...
        Double D1 = Double.parseDouble(mPSalesBrandBasedProductModel.getTargtVal());
        double scale1 = Math.pow(10, 3);
        myViewHolder.tv_target.setText("₹ " + Math.round(D1 * scale1) / scale1 + "L");

        Double D2 = Double.parseDouble(mPSalesBrandBasedProductModel.getCSaleVal());
        double scale2 = Math.pow(10, 3);
        myViewHolder.tv_primary.setText("₹ " + Math.round(D2 * scale2) / scale2 + "L");

        myViewHolder.tv_achie.setText(" " + mPSalesBrandBasedProductModel.getAch() + "%");
        myViewHolder.tv_growth.setText(" " + mPSalesBrandBasedProductModel.getGrowth() + "%");

        myViewHolder.tr_pcpm.setVisibility(View.VISIBLE);
        Double D3 = Double.parseDouble(mPSalesBrandBasedProductModel.getPCPM());
        double scale3 = Math.pow(10, 3);
        myViewHolder.tv_pcpm.setText(" " + Math.round(D3 * scale3) / scale3 + "%");
//        myViewHolder.tv_pcpm.setText(" " + mPSalesBrandBasedProductModel.getPCPM() + "%");
        ...
        Entry1.add(new BarEntry(0,Float.parseFloat(mPSalesBrandBasedProductModel.getTargtVal())));
        Entry2.add(new BarEntry(1,Float.parseFloat(mPSalesBrandBasedProductModel.getCSaleVal())));
        ...
        myViewHolder.BarGraph.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                showPopup(pSalesBrandBasedProductModel.get(i).getTargtVal(), pSalesBrandBasedProductModel.get(i).getCSaleVal());
            }
        });
    }
    ...
    public int getItemCount() {
        return pSalesBrandBasedProductModel.size();
    }

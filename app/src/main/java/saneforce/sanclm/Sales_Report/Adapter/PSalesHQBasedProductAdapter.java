...
public class PSalesHQBasedProductAdapter extends RecyclerView.Adapter<PSalesHQBasedProductAdapter.MyViewHolder> {
    public ArrayList<PSalesHQBasedProductModel> mPSalesHQBasedProductModel;
    public Context context;
    public static String Target,Sale;

    public PSalesHQBasedProductAdapter(ArrayList<PSalesHQBasedProductModel> mPSalesHQBasedProductModel, Context context) {
        this.mPSalesHQBasedProductModel = mPSalesHQBasedProductModel;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder myViewHolder, int i) {
        final PSalesHQBasedProductModel mmPSalesHQBasedProductModel = mPSalesHQBasedProductModel.get(i);
        myViewHolder.tv_dot.setTextColor(Color.rgb(0, 191, 255));
        myViewHolder.tv_achie.setTextColor(Color.rgb(0, 191, 255));
        myViewHolder.tv_HQName.setText(mmPSalesHQBasedProductModel.getProduct_Name());
...
//        myViewHolder.tv_primary.setText("₹ " + mPSalesHQBasedBrandModel.getPrimary() + "L");
        ...
        Double D1 = Double.parseDouble(mmPSalesHQBasedProductModel.getTargtVal() );
        double scale1 = Math.pow(10, 3);
        myViewHolder.tv_target.setText("₹ " + Math.round(D1 * scale1) / scale1 + "L");

        Double D2 = Double.parseDouble(mmPSalesHQBasedProductModel.getCSaleVal());
        double scale2 = Math.pow(10, 3);
        myViewHolder.tv_primary.setText("₹ " + Math.round(D2 * scale2) / scale2 + "L");

        myViewHolder.tv_achie.setText(" " + mmPSalesHQBasedProductModel.getAch() + "%");
        myViewHolder.tv_growth.setText(" " + mmPSalesHQBasedProductModel.getGrowth() + "%");

        myViewHolder.tr_pcpm.setVisibility(View.VISIBLE);
//        myViewHolder.tv_pcpm.setText(" " + mmPSalesHQBasedProductModel.getPCPM() + "%");
        Double D3 = Double.parseDouble(mmPSalesHQBasedProductModel.getPCPM());
        ...
        Entry1.add(new BarEntry(0,Float.parseFloat(mmPSalesHQBasedProductModel.getTargtVal())));
        Entry2.add(new BarEntry(1,Float.parseFloat(mmPSalesHQBasedProductModel.getCSaleVal())));
        ...
        myViewHolder.BarGraph.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                showPopup(mPSalesHQBasedProductModel.get(i).getTargtVal(), mPSalesHQBasedProductModel.get(i).getCSaleVal());
            }
        });
    }
    ...
    public int getItemCount() {
        return mPSalesHQBasedProductModel.size();
    }
